...
ROOT_TO_PC = {root: i for i, root in enumerate(ROOTS)}
CHORD_INTERVALS = {
    'maj': [0, 4, 7],
    'min': [0, 3, 7],
    'dim': [0, 3, 6],
...
def sec_to_tick(sec: float, tempos, tpq: int) -> int:
    """Convert seconds to MIDI ticks using the score tempo map."""
    if not tempos:
        return int(sec * tpq * 2)  # fallback: 120 BPM

    tempo_events = sorted(tempos, key=lambda t: t.time)
    current_sec = 0.0

    for i, tempo in enumerate(tempo_events):
        start_tick = tempo.time
        qpm = float(tempo.qpm)
        next_tick = tempo_events[i + 1].time if i + 1 < len(tempo_events) else None

        if next_tick is None:
            return int(start_tick + (sec - current_sec) * qpm / 60.0 * tpq)

        segment_seconds = (next_tick - start_tick) / tpq * 60.0 / qpm
        if sec < current_sec + segment_seconds:
            return int(start_tick + (sec - current_sec) * qpm / 60.0 * tpq)
        current_sec += segment_seconds

    return int(sec * tpq * 2)


def get_bar_ticks(score) -> int:
    """Use the MIDI time signature instead of assuming every song is 4/4."""
    tpq = score.ticks_per_quarter
    if score.time_signatures:
        ts = sorted(score.time_signatures, key=lambda t: t.time)[0]
        beats_per_bar = ts.numerator * (4.0 / ts.denominator)
        return max(1, int(round(tpq * beats_per_bar)))
    return tpq * 4


def chord_pitch_classes(chord_label: str) -> Optional[set]:
    """Return chord tones as pitch classes for simple consistency analysis."""
    if not chord_label or chord_label == 'N' or ':' not in chord_label:
        return None
    root, quality = chord_label.split(':', 1)
    if root not in ROOT_TO_PC or quality not in CHORD_INTERVALS:
        return None
    root_pc = ROOT_TO_PC[root]
    return {(root_pc + interval) % 12 for interval in CHORD_INTERVALS[quality]}


def parse_chord_file(chord_path: str, tpq: int, tempos=None) -> List[Tuple[int, int, str]]:
    ...
    with open(chord_path) as f:
        for line in f:
            ...
            start_tick = sec_to_tick(start_sec, tempos, tpq)
            end_tick = sec_to_tick(end_sec, tempos, tpq)
...
def velocity_to_bin(velocity: int) -> int:
    """Quantize MIDI velocity to a compact symbolic token."""
...
def melody_to_tokens(notes, bar_start: int, bar_end: int, token2id: dict) -> List[int]:
    ...
    for note in bar_notes:
        ...
        tokens.append(token2id[f'Velocity_{velocity_to_bin(note.velocity)}'])
...
def process_song(midi_path: str, chord_path: str, token2id: dict) -> Optional[List[int]]:
    ...
    tpq = score.ticks_per_quarter
    bar_ticks = get_bar_ticks(score)

    # Parse chords
    chords = parse_chord_file(chord_path, tpq, score.tempos)
    if not chords:
        return None
...
def resolve_pop909_dir(data_dir: str) -> str:
    """Accept either POP909-Dataset root or the inner POP909 folder."""
    if os.path.isdir(os.path.join(data_dir, 'POP909')):
        return os.path.join(data_dir, 'POP909')
    return data_dir
...
def generate_melody_forced_chords(model, chord_sequence: List[str], token2id: dict, id2token: dict,
                                   ...
                                   top_k: int = 50, min_notes_per_bar: int = 2,
                                   max_tokens_per_bar: int = 64) -> List[int]:
    ...
    At each bar boundary, we force-inject the next chord token. During
    sampling, structural tokens are suppressed until a bar has enough notes,
    which avoids very sparse generations dominated by early BAR/EOS/Rest.
    """
    model.eval()

    tokens = [token2id['BOS']]
    structural_ids = [
        idx for idx, tok in id2token.items()
        if tok.startswith('Chord_') or tok in {'BAR', 'EOS'}
    ]
    rest_id = token2id.get('Rest')
    ...
    for bar_idx, chord in enumerate(chord_sequence):
        ...
        max_ctx = 512  # match training max_seq_len
        notes_in_bar = 0
        for step in range(max_tokens_per_bar):
            ...
            if notes_in_bar < min_notes_per_bar:
                logits[structural_ids] = float('-inf')
                if rest_id is not None:
                    logits[rest_id] = float('-inf')
...
def analyze_generated(token_ids: List[int], id2token: dict) -> dict:
    ...
    for tid in token_ids:
        t = id2token.get(tid, '')
        if t.startswith('Chord_'):
            current_chord = t[len('Chord_'):]
        elif t.startswith('Pitch_'):
            pitch = int(t.split('_')[1])
            pitches.append(pitch)
            n_notes += 1
            chord_pcs = chord_pitch_classes(current_chord)
    ...
    if chord_tone_total:
        stats['chord_tone_ratio'] = chord_tone_hits / chord_tone_total
    if pitches:
...
def tokens_to_midi(token_ids: List[int], id2token: dict, output_path: str,
    ...
    current_velocity = 80
    last_end_by_pitch = defaultdict(float)
    ...
    for tid in token_ids:
        ...
        if token.startswith('TimeShift_'):
            ...
            if bar_index > 0:
                bar_start = (bar_index - 1) * bar_beats
                bar_end = bar_index * bar_beats
                current_time = min(max(current_time, bar_start), bar_end - beat_unit)
            ...
            current_velocity = int((vel_bin + 0.5) / VELOCITY_BINS * 127)
            ...
            if current_pitch is not None:
                if bar_index > 0:
                    bar_end = bar_index * bar_beats
                    current_time = min(current_time, bar_end - beat_unit)
                    current_duration = min(current_duration, max(beat_unit, bar_end - current_time))
                start_time = max(current_time, last_end_by_pitch[current_pitch])
                midi.addNote(track, 0, current_pitch, start_time, current_duration,
                             current_velocity)
                last_end_by_pitch[current_pitch] = start_time + current_duration
            current_pitch = None
            current_duration = None

        elif token == 'BAR':
            current_time = bar_index * bar_beats
            bar_index += 1
...
def main():
    ...
    parser.add_argument('--data_dir', type=str,
                        default=r'D:\UCSD\SP26\CSE253\Assignment\2\dataset\POP909-Dataset',
    ...
    parser.add_argument('--min_notes_per_bar', type=int, default=2,
                        help='Minimum generated Pitch tokens before a forced-chord bar may end')
    parser.add_argument('--max_tokens_per_bar', type=int, default=64,
                        help='Maximum melody tokens sampled for each forced-chord bar')
    ...
    data_dir = resolve_pop909_dir(args.data_dir)
    print(f"Using POP909 data from: {data_dir}")
    train_seqs, test_seqs = prepare_dataset(data_dir, token2id)
    ...
    generated = generate_melody_forced_chords(
        ...
        max_tokens_per_bar=args.max_tokens_per_bar
    )
